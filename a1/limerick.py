...
class LimerickDetector:
    ...
    def rhymes(self, a, b):
        ...
        for pra in ListOf_Prlists_a:
            for prb in ListOf_Prlists_b:
                ...
                if a_vStart == -1 or a_vStart == -1 : return False
    ...
    def is_limerick(self, text):
        ...
        if len(linesList) != 5: return False  
        # PUNCTUTATION ON WORDS ???
    
        #Tokenize
        lines_tokenized = map ( lambda line : word_tokenize(line), linesList )
        
        #Clean tokens of special characters
        for i in range( len(lines_tokenized)):
            newTokens = []
            for token in lines_tokenized[i]:
                if len(token)==1 and token in punctuation: continue
                newTokens.append(token)
            lines_tokenized[i] = newTokens    
        
        A_lines_tokenized = [lines_tokenized[i] for i in [0,1,4]]  
        B_lines_tokenized = [lines_tokenized[i] for i in [2,3]]
...
# The code below should not need to be modified
    ...
    def guess_syllables(self, word):
        ...
        if wordList[0] in vowelDict:
            syllCount+=1
            if len(wordList)==1: return syllCount
        
        prev = wordList[0]
        for curr in wordList[1:]:
            if (prev not in vowelDict) and (curr in vowelDict): 
                    syllCount+=1
            prev = curr 
                   
        if (wordList[-1] == 'y' and wordList[-2] not in vowelDict ) or (wordList[0] =="'"): syllCount+=1
        if (wordList[-1] == 'e'): syllCount-=1
        
        # le, les check
        if len(wordList)>=3:
            if word[-2:] =='le'  and (wordList[-3] not in vowelDict) : syllCount+=1
            if word[-3:] =='les' and (len(wordlist)>=4 and wordList[-4] not in vowelDict): syllCount+=1 
